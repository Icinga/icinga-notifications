	...
	for {
		...
		case ev, ok := <-catchupEventCh:
			...
			if len(catchupBuffer) > 0 {
				...
				if !ok {
					client.Logger.Errorw("Event to be replayed is not in cache", zap.Stringer("event", ev))
...
// Process incoming objects and reconnect to the Event Stream with catching up on missed objects if necessary.
//
