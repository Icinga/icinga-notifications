...
func (client *Client) buildAcknowledgementEvent(ctx context.Context, host, service, author, comment, evType string) (*event.Event, error) {
	...
	if !(evType == event.TypeAcknowledgementSet || evType == event.TypeAcknowledgementRemoved) {
...
func (client *Client) buildDowntimeEvent(ctx context.Context, d Downtime, evType string) (*event.Event, error) {
	...
	switch evType {
	case event.TypeDowntimeStart, event.TypeDowntimeRemoved, event.TypeDowntimeEnd, event.TypeDowntimeAdded, event.TypeDowntimeTriggered:
	default:
