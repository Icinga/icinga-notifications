...
func doProcess(
	...
	err = entries.Evaluate(res.RuntimeConfig, filterContext, rules, config.EvalOptions{
	...
	if err := notification.NotifyContacts(ctx, res, req, notifications); err != nil {
		res.Logger.Errorw("Failed to send all pending notifications", zap.Error(err))
		return err
	}
	return nil
}
...
func loadNotificationsFromEntries(
	...
	for _, route := range entries {
		...
		if len(channels) == 0 {
			res.Logger.Warnw("Rule routing expanded to no contacts",
				zap.Object("rule_routing", route))
			continue
		}
