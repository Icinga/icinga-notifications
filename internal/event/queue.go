...
var (
	...
	// errEnvelopeVersionToNew is returned when a queue entry's envelope version is newer than the version supported by this node.
	errEnvelopeVersionToNew = errors.New("envelope version is too new")
)
...
type (
	...
	QueueCallbacks struct {
		...
		GenObjectID func(int64, map[string]string) types.Binary
...
func (q *Queue) toEvent() (*Event, error) {
	...
	if q.Envelope.Version > EnvelopeEventVersion {
		return nil, fmt.Errorf("%w: got %d, supported <= %d",
			errEnvelopeVersionToNew, q.Envelope.Version, EnvelopeEventVersion)
	}
...
func ProcessQueue(ctx context.Context, db *database.DB, logger *logging.Logger, cbs QueueCallbacks) error {
	...
	// updStmt is used to update the state of a queue entry state and its last_update after processing.
	...
	// lockStmt is used to insert a lock for the target object of a queue entry, ensuring that no other node
	// is processing a queue entry for the same object concurrently. The lock is released after processing,
	// regardless of success or failure, unless the context is canceled, in which case the retention.ResetPruner
	// will eventually unlock it at some point in the future.
	lockStmt, _ := db.BuildInsertIgnoreStmt(new(jobProcessingLock))
				...
				for {
					...
					for _, q := range candidates {
						...
						case errors.Is(err, errClaimLost):
							// The candidate could not be claimed because its target object is already locked,
							// or because a concurrent node claimed the same entry first.
