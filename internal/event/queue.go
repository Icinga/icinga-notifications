...
type (
	...
	Queue struct {
		...
		// It is used to deduplicate events and ensure that the same event is not processed multiple times.
		ID types.UUID `db:"id"`
	...
	jobProcessingLock struct {
		...
		QueueID  types.UUID   `db:"job_queue_id"`
	}
)
...
func (q *Queue) toQuickAction() (*QuickAction, error) {
	...
	if q.Envelope.Version > EnvelopeQAVersion {
		return nil, fmt.Errorf("%w: got %d, supported <= %d",
			errEnvelopeVersionTooNew, q.Envelope.Version, EnvelopeQAVersion)
	}
...
func Enqueue(ctx context.Context, db *database.DB, ev *Event) error {
	...
	err := json.NewEncoder(&jsonBuff).Encode(ev)
	if err != nil {
		return fmt.Errorf("cannot JSON encode event.Event: %w", err)
	}

	if !ev.ID.Valid {
		ev.ID = types.MakeUUID(uuid.NewHash(sha256.New(), uuid.Nil, jsonBuff.Bytes(), 8))
	}
