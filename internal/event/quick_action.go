...
type QuickAction struct {
	...
	EventTypeWhitelist types.StringList  `json:"event_type_whitelist"`
}
...
func (qa *QuickAction) MarshalLogObject(enc zapcore.ObjectEncoder) error {
	...
	err := enc.AddArray("whitelist", zapcore.ArrayMarshalerFunc(func(enc zapcore.ArrayEncoder) error {
		if qa.EventTypeWhitelist.Valid {
			for _, v := range qa.EventTypeWhitelist.Elements {
				enc.AppendString(v)
			}
		}
		return nil
