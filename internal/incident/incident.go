...
// DoQuickAction processes the given quick action for the current incident in an own transaction.
func (i *Incident) DoQuickAction(ctx context.Context, qa *event.QuickAction) error {
	...
	return i.db.ExecTx(ctx, &sql.TxOptions{Isolation: sql.LevelReadCommitted}, func(ctx context.Context, tx *sqlx.Tx) error {
		i.ObjectID = object.ID(qa.SourceID, qa.ObjectTags)
		if err := i.RestoreState(ctx, tx, true); err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				return nil // The incident is already recovered, so nothing to do.
			}

			i.logger.Warnw("Failed to restore incident row from the database",
				zap.Stringer("action", qa.Kind),
				zap.Error(err))
			return fmt.Errorf("cannot restore incident state: %w", err)
		}
		...
		case event.ActionManage:
			if i.HasManager() {
				i.logger.Warnw("Cannot add recipient as manager, incident already has a manager", zap.Stringer("recipient", r))
				return errors.New("incident already has a manager")
			}

			if err := i.addRecipient(ctx, tx, r, RoleManager); err != nil {
				i.logger.Warnw("Failed to add recipient as manager", zap.Stringer("recipient", r), zap.Error(err))
				return fmt.Errorf("cannot add recipient as manager: %w", err)
			}
			...
			if err != nil {
				i.logger.Warnw("Failed to retrieve object from the database", zap.Error(err))
				return err
			}
		...
		case event.ActionUnmanage, event.ActionSubscribe:
			if qa.Kind == event.ActionUnmanage && !i.HasManager() {
				i.logger.Warnw("Cannot remove recipient as manager, incident has no manager", zap.Stringer("recipient", r))
				return errors.New("incident already has no manager")
			}

			if err := i.addRecipient(ctx, tx, r, RoleSubscriber); err != nil {
				i.logger.Warnw("Failed to add/demote recipient as subscriber",
					zap.Stringer("recipient", r),
					zap.Stringer("action", qa.Kind),
					zap.Error(err))
				return fmt.Errorf("cannot add recipient as subscriber: %w", err)
			}
			return nil
		...
		case event.ActionUnsubscribe:
			...
			if !exists {
				i.logger.Warnw("Cannot unsubscribe recipient, recipient is not subscribed", zap.Stringer("recipient", r))
				return errors.New("recipient is not subscribed")
			}
			...
			if _, err := tx.NamedExecContext(ctx, query, &ContactRow{Key: recipientKey, IncidentID: i.ID()}); err != nil {
				i.logger.Warnw("Failed to unsubscribe recipient", zap.Stringer("recipient", r), zap.Error(err))
				return fmt.Errorf("cannot unsubscribe recipient: %w", err)
			}
			return i.recordRecipientRoleChange(ctx, tx, r, state.Role, RoleNone)
