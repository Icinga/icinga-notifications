...
// Warmup loads all active (not yet closed) incidents from the database and restores all their states.
// Returns error ony database failure.
func Warmup(
	ctx context.Context, db *icingadb.DB, logger *logging.Logger, runtimeConfig *config.RuntimeConfig,
	...
	for _, incidentRow := range incidentRows {
		...
		incident, _, err := GetCurrent(ctx, db, obj, logger, runtimeConfig, configFile, false)
		if err != nil {
			continue
		}
...
func GetCurrent(
	...
	if currentIncident == nil {
		...
		incident := NewIncident(db, obj, runtimeConfig, incidentLogger, configFile)
