...
func TestPlugin(t *testing.T) {
	...
	require.NoError(t, os.Setenv("ICINGA_NOTIFICATIONS_ICINGAWEB2_URL", "http://localhost")) // Is required, so populate it.
	...
	t.Run("Plugin State Management", func(t *testing.T) {
		...
		stateBefore, err := getStateByChannelID(t.Context(), db, ch.ID, types.UnixMilli(time.Now().Add(-5*time.Minute)))
		...
		require.ErrorContains(t, plugin1.SendNotification(t.Context(), req), "plugin slept for 1s")
		stateAfter, err := getStateByChannelID(t.Context(), db, ch.ID, types.UnixMilli(time.Now().Add(-5*time.Minute)))
		require.NoError(t, err)
		...
		states, err := getStateByChannelID(t.Context(), db, ch.ID, types.UnixMilli(time.Now().Add(-5*time.Minute)))
		require.NoError(t, err)
		require.Len(t, states, 1)
		plugin1.highestSeenChangedAt = types.UnixMilli(time.Now().Add(-5 * time.Minute)) // Reset the highestSeenChangedAt to simulate a new state change.
	...
	t.Run("Type Validation", func(t *testing.T) {
		...
		assert.Error(t, ValidateType("invalid-type"))
...
func makeTestRequest(addrType string, muted, recovered bool) *plugin.NotificationRequest {
	return &plugin.NotificationRequest{
		...
		Incident: &plugin.Incident{Id: time.Now().UnixNano(), IsMuted: muted, IsRecovered: recovered},
...
func cleanupDB(ctx context.Context, db *database.DB, t *testing.T) {
	...
	case database.PostgreSQL:
		...
		_, err := db.ExecContext(ctx, `TRUNCATE TABLE available_channel_type RESTART IDENTITY CASCADE`)
		require.NoError(t, err)
	case database.MySQL:
