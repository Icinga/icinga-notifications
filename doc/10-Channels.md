...
The `fields` array must contain an even number of elements, where each pair of elements represents a key-value pair.
Also, the request's `id` field must be omitted, as the request is expected to be a
...
The Icinga Notifications will call this method at least once on each channel before sending the first notifications to
...
care of setting up a JSON-RPC server and dispatching of requests to the appropriate methods of the channel plugin.
